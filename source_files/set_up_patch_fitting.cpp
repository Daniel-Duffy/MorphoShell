...
void set_up_patch_fitting(
    ...
    for( auto& tri: triangles ){
        ...
        // Put closest two nodes in non_vertex_patch_nodes_ids.
        for(int p = 0; p < 2; ++p){
            tri.non_vertex_patch_nodes_ids[p] = possible_patch_node_ids[idx_in_dist_list[p]];
        }
        ...
        for(size_t q = 2; q < possible_patch_node_ids.size(); ++q){
            ...
            */
            ...
            if( (!(temp_abs_cond_num_divided_by_thresh_val < 1.0)) || mat_is_invertible == false ){
                ...
                if(q == possible_patch_node_ids.size()-1) {
