...
def boundedges(p, t):
    # dld34: I have written my own version of this function, which works in the 
    # same way, but uses more up to date numpy features to provide something 
    # more readable and less hacky. It's in my mesh_processing_module.py.
    ...
    edges = np.vstack((t[:,[0,1]],
                       t[:,[0,2]],
                       t[:,[1,2]]))
    #dld34: node3 is a vector, with one element for each edge in edges, whose 
    # contents is the label of the node in the triangle that corresponds 
    # to that entry in edges, which is not either of the 2 nodes in the 
    # corresponding edge.
    node3 = np.hstack((t[:,2],t[:,1],t[:,0]))
    edges.sort(1)
    #dld34: nl.unique is a custom made function, which I think is now 
    # redundant, with all its features now built into numpy.
    _, ix, jx = ml.unique_rows(edges, True, True)
    #dld34: using np.histogram here is another redundant hack, to count how 
    # many occurences of each edge have been found (how many times each row of 
    # the unique-rows matrix must be spat out in reproducing the original 
    # edges array). Then the indices (jx, in the unique-rows array) where this 
    # number is 1 (i.e. vec==1) gives qx, the indices in the unique-rows 
    # matrix (AND HENCE ix) that correspond to boundary edges. So then 
    # e = edges[ix[qx]] leaves e holding just the boundary edges. Then only the 
    # corresponding entries of node3 are retained.
    vec, _ = np.histogram(jx, np.arange(max(jx)+2))
    qx = (vec == 1).nonzero()
    e = edges[ix[qx]]
    node3 = node3[ix[qx]]

    #dld34: This then just looks at each boundary edge, and another vector from
    # the second node of the edge to the 'other' node in the triangle, and 
    # finds the sign of the z component of the cross product. If it's positive, 
    # the nodes in the edge are swapped, so that all the boundary edges have a 
    # consistent orientation going around the boundary/boundaries.
